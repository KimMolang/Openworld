...
void VIBufferModel::Render()
{
	VIBuffer::Render();
}

void VIBufferModel::InitRasterizerState()
{
...
void VIBufferModel::MakeVertexNormal(BYTE* _pVertices, WORD* _pIndices)
{
	if (!_pIndices)
...
void VIBufferModel::SetNormalVectorByBasic(BYTE * _pVertices)
{
	D3DXVECTOR3		vNormal;
	VertexTexture* pVertex = nullptr;
...
void VIBufferModel::SetNormalVectorByAverage(BYTE * _pVertices, WORD * _pIndices, int _iPrimitives, int _iOffset, bool _bStrip)
{
...
D3DXVECTOR3 VIBufferModel::GetTriAngleNormal(BYTE* _pVertices, USHORT _nIndex_0, USHORT _nIndex_1, USHORT _nIndex_2)
{
	D3DXVECTOR3 vNormal(0.f, 0.f, 0.f);
